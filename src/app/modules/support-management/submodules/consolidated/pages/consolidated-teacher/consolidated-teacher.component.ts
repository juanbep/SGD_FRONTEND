...
export class ConsolidatedTeacherComponent implements OnInit {
  ...
  userEffec = effect(() => {
    this.consolidatedServicesService.getFilterActivitiesParams();
    this.currentPage = 1;
  ...
  recoverInfoTeacher(): void {
    if (this.idUserTeacher) {
      this.consolidatedServicesService
        .getInfoTeacher(this.idUserTeacher)
        .subscribe({
          next: (response) => {
    ...
    if (this.idUserTeacher) {
      this.consolidatedServicesService
        .getConsolidatedActitiesTeacherByParams(
          ...
          page - 1,
          SIZE_PAGE,
          paramsFilter.activityType || '',
          paramsFilter.activityName || '',
          paramsFilter.sourceType || '',
          paramsFilter.sourceState || ''
        )
        .subscribe({
          next: (response) => {
            this.consolidatedTeacher = response.data;
            this.consolidatedServicesService.setDataConsolidatedTeacher(
              response.data
            );
          },
          error: (error) => {
  ...
  responseApproveConsolidated(response: boolean): void {
    ...
    if (response && this.idUserTeacher && this.infoCurrentUser) {
      ...
      this.consolidatedServicesService
        .saveConsolidated(
          this.idUserTeacher,
          this.infoCurrentUser.oidUsuario,
          ''
        )
        .subscribe({
          next: (response) => {
  ...
  downloadAllSuppotFiles() {
    this.consolidatedServicesService
      .downloadAllSupportFiles(
        this.infoDataTeacher?.periodoAcademico || '',
        this.infoDataTeacher?.departamento || '',
        this.infoDataTeacher?.tipoContratacion || '',
        this.idUserTeacher || 0,
        null
      )
      .subscribe({
        next: (response: any) => {
