...
export class CpdWordGeneratorService {
  ...
  async generateWordDocument(
    teacherConsolidated: UsuarioResponse,
    ...
    infoConsolidated: {
      ...
      oficioNumber: string;
      oficioDate: string;
      meetingCouncil: string;
      councilPresident: string;
    }
  ) {
    const imageData = await this.getImage('assets/images/logo-unicauca-2.png');

    this.currentDateDay = new Date().toLocaleDateString('es-ES', {
      day: 'numeric',
    });
    this.currentDateMonth = new Date().toLocaleDateString('es-ES', {
      month: 'long',
    });
    this.currentDateYear = new Date().toLocaleDateString('es-ES', {
      year: 'numeric',
    });
    this.oficioDate = this.changeDateFormat(
      new Date(infoConsolidated.oficioDate)
    );
    this.meetingCouncil = this.changeDateFormat(
      new Date(infoConsolidated.meetingCouncil)
    );
        ...
        },
        {
          children: [
            ...
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: `El(la) profesor(a) ${teacherConsolidated.nombres} ${
                    teacherConsolidated.apellidos
                  }, obtuvo una evaluación ${this.getDescritionQualification(
                    averageEvaluation
                  )}, la cual fue calificada con ${averageEvaluation.toFixed(
                    2
                  )} puntos.`,
                  size: 24,
                }),
            ...
            new Paragraph({
              ...
              children: [
                ...
                new TextRun({
                  text: `Evaluar el desempeño del (la) profesor(a) ${
                    teacherConsolidated.nombres
                  } ${teacherConsolidated.apellidos} de ${
                    teacherConsolidated.usuarioDetalle.dedicacion
                  } en la categoría de ${
                    teacherConsolidated.usuarioDetalle.categoria
                  }, adscrito al ${
                    teacherConsolidated.usuarioDetalle.departamento
                  }, con una calificación cuantitativa de ${averageEvaluation.toFixed(
                    2
                  )} puntos equivalentes a una calificación cualitativa ${this.getDescritionQualification(
                    averageEvaluation
                  )}, para los periodos académicos correspondientes al ${
                    infoConsolidated.resolutionDate
                  }.`,
                  size: 24,
                }),
              ],
            ...
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: `Dada en Popayán, a los ${this.dayInWords(
                    parseInt(this.currentDateDay)
                  )} (${this.currentDateDay}) días del mes de ${
                    this.currentDateMonth
                  } del ${this.anioEnPalabra(
                    parseInt(this.currentDateYear)
    ...
    Packer.toBlob(document).then((blob) => {
      saveAs(
        blob,
        `Resolución_$${teacherConsolidated.nombres} ${teacherConsolidated.apellidos}.docx`
      );
    });
  }
