...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, PaginatorComponent, CpdInfoFormComponent],
  templateUrl: './user-table.component.html',
  styleUrl: './user-table.component.css',
})
export class UserTableComponent {
  @ViewChild(CpdInfoFormComponent)
  cpdInfoFormComponent: CpdInfoFormComponent | null = null;
  ...
  public userSelectedToCreateRelution: ConsolidadoHistoricoResponse | null =
    null;
  ...
  findEvaluationByAcademicPeriod(
    academicPeriod: number,
    consolidadoHistorico: ConsolidadoHistoricoResponse
  ): number | null {
    const evaluation = consolidadoHistorico.calificacionesPorPeriodo.find(
      (evaluation) => evaluation.idPeriodoAcademico === academicPeriod
    );
  ...
  public getAverageEvaluation(
    ...
    const evaluations = consolidadoHistorico.calificacionesPorPeriodo.map(
      (evaluation) => evaluation.calificacion
    );
  ...
  wordGenerator(
    ...
    let infoTeacherConsolidated: UsuarioResponse | null = null;
    this.historicalServices.getUserById(teacherInfo.oidUsuario).subscribe({
      next: (response) => {
