...
export class ResponsibilitiesEditCoordinatorFormComponent implements OnInit {
  ...
  loadResponsibility(id: number) {
    if (id) {
      this.responsibilitiesServicesService
        .getInforResponsibilityByFormCoordinator(id)
        .subscribe({
          next: (source) => {
  ...
  pathForm() {
    const informes = this.sourceResponsibility?.informesAdministracion || [];
    informes.forEach(({ oidObjetivoComponente, calificacion }) => {
      const fieldName = `qualification_${oidObjetivoComponente}`;
      this.formEvaluation.get(fieldName)?.setValue(calificacion);
    });

    this.formEvaluation
      .get('observations')
      ?.setValue(this.sourceResponsibility?.Fuente.observacion || '');
  }
