...
export class ActivitiesFilterComponent {
  ...
  ngOnInit(): void {
    this.recoverCatalog();
    const filterParams =
      this.activitiesServicesService.getParamsActivitiesFilterSignal();
    this.formFilter
      .get('activityType')
      ?.setValue(filterParams.activityType || '');
    this.formFilter
      .get('activityName')
      ?.setValue(filterParams.activityName || '');
    this.formFilter
      .get('evaluatorName')
      ?.setValue(filterParams.evaluatorName || '');
    this.formFilter
      .get('evaluatorRole')
      ?.setValue(filterParams.evaluatorRole || '');
  }
