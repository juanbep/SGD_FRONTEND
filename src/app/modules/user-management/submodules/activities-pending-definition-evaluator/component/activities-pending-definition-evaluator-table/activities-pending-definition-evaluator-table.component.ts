...
export class ActivitiesPendingDefinitionEvaluatorTableComponent {
  private activitiesPendingDefinitionEvaluatorService = inject(
  ...
  activitiesEffect = effect(() => {
    this.filterParams =
      this.activitiesPendingDefinitionEvaluatorService.getParamsActivitiesFilter();
    this.currentPage = 1;
  ...
  recoverActivities(page: number, size: number) {
    this.activitiesPendingDefinitionEvaluatorService
      .getResponsibilities(
