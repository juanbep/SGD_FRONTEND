...
export class UsersTableComponent implements OnInit {
  @ViewChild(ModalUserDetailslComponent)
  modalUserDetails: ModalUserDetailslComponent | null = null;
  ...
  getAllUsers(page: number, totalPage: number) {
    ...
    this.usersService
      .getUserByParams(
        page - 1,
        totalPage,
        paramsFilter?.identification || '',
        paramsFilter?.nameUser || '',
        paramsFilter?.faculty || '',
        paramsFilter?.program || '',
        '',
        '',
        '',
        '',
        ROLES.DOCENTE.toString(),
        paramsFilter?.state || ''
      )
      .subscribe({
        next: (response) => {
