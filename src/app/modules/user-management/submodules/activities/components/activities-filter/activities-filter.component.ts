...
export class ActivitiesFilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const paramsFilter =
      this.activityManagementService.getParamsActivitiesFilter();
    this.formFilter
      .get('activityType')
      ?.setValue(paramsFilter?.typeActivity || '');
    this.formFilter
      .get('activityName')
      ?.setValue(paramsFilter?.nameActivity || '');
  }
  ...
  searchActivities() {
    ...
    this.activityManagementService.setParamsActivitiesFilter({
      nameActivity: activityName,
      typeActivity: activityType,
      activityCode: activityCode,
      administrativeCode: administrativeAct,
      vriCode: vriCode,
    });
  }
