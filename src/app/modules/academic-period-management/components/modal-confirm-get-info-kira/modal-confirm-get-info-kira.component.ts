...
export class ModalConfirmGetInfoKiraComponent implements OnInit {
  ...
  validateWithBoosDepartmentDoesNotExist() {
    if (
      !this.bossDepartament.find(
        (boss) => boss.usuarioDetalle.departamento === DEPARTAMENTOS.TELEMATICA
      )
    ) {
    ...
    if (
      !this.bossDepartament.find(
        (boss) =>
          boss.usuarioDetalle.departamento === DEPARTAMENTOS.TELECOMUNICACIONES
      )
    ) {
    ...
    if (
      !this.bossDepartament.find(
        (boss) =>
          boss.usuarioDetalle.departamento ===
          DEPARTAMENTOS.ELECTRONICA_INSTRUMENTACION_Y_CONTROL
      )
    ) {
      this.departmentWithoutBoss.push(
        DEPARTAMENTOS.ELECTRONICA_INSTRUMENTACION_Y_CONTROL
      );
    }
    if (
      !this.bossDepartament.find(
        (boss) => boss.usuarioDetalle.departamento === DEPARTAMENTOS.SISTEMAS
      )
    ) {
  ...
  isFormValid(): boolean {
    return (
      this.form.valid &&
      this.form.get('checkbox1')?.value &&
      this.form.get('checkbox2')?.value &&
      this.form.get('checkbox3')?.value &&
      this.form.get('checkbox4')?.value &&
      this.form.get('checkbox5')?.value
    );
  }

  open() {
    const modal = new bootstrap.Modal(
      document.getElementById('modal-confirm-dialog-kira')
    );
    if (modal) {
