...
export class UmUsersServicesService {
  ...
  getAllUsers(
    page: number,
    totalPage: number
  ): Observable<SimpleResponse<PagedResponse<UsuarioResponse>>> {
    const params: Params = {
      page: page.toString(),
      size: totalPage.toString(),
    };
    return this.httpClient.get<SimpleResponse<PagedResponse<UsuarioResponse>>>(
  ...
  getUserbyId(id: number): Observable<SimpleResponse<UsuarioResponse>> {
    return this.httpClient.get<SimpleResponse<UsuarioResponse>>(
  ...
  updateUsers(
    idUsuario: number,
    user: UsuarioCreate
  ): Observable<UsuarioCreate> {
    return this.httpClient.put<UsuarioCreate>(
      `${this.baseUrl}/api/usuarios/${idUsuario}`,
      user
    );
  }
}
