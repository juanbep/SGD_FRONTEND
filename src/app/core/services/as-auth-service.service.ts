...
export class AsAuthServiceService {
  ...
  login(userEmail: string, password: string): number | null {
    if (
      this.usersLogin.find(
        (user) => user.email === userEmail && user.pass === password
      )
    ) {
      ...
      sessionStorage.setItem(
        'idUser',
        this.usersLogin
          .find((user) => user.email === userEmail && user.pass === password)
          ?.idUser.toString() || ''
      );
      return (
        this.usersLogin.find(
  ...
  getUserInfo(): Observable<SimpleResponse<UsuarioResponse>> {
    return this.http.get<SimpleResponse<UsuarioResponse>>(
      `${this.baseUrlUserInfo}/sgd-back/api/usuarios/logueado`
    );
  }
